...
BOT_TOKEN = "5127516174:AAFVrnV9hUlCwGy1bT9Z5ns_EDUchAbI0uk"

PAYMENT_CHANNEL = "@aviaxsupport"
OWNER_ID = 5201374096
CHANNELS = ["@aviaxsupport"]
Mini_Withdraw = 1
Paytmkeys = "key"
mid = "key"
paytmtoken = "key"
    ...
    if number == '+91' or numberphone == "91":
        ...
        if user not in data['refer']:
            ...
            if int(data['referby'][user]) != user_id:
                ...
                json.dump(data, open('paytmusers.json', 'w'), indent=4)
                botdata = json.load(open('panel.json', 'r'))
                Per_Refer = float(round( random.uniform(0.30 , 0.75),2))
    ...
    if bdata['contact'][str(message.chat.id)] == True:
      ...
      if ch is True:
        ...
        if message.text == '🙌🏻 Invite':
            ...
            user = str(user_id)
            botdata = json.load(open('panel.json', 'r'))
            Per_Refer = "Random refer bonus 0.30 upto 1.00🥰"
            if user not in data['referred']:
                data['referred'][user] = 0
            json.dump(data, open('paytmusers.json', 'w'), indent=4)
            time.sleep(0.8)
        ...
        if message.text == "🎁 Bonus":
            botdata = json.load(open('panel.json', 'r'))
            Daily_bonus =  round(random.uniform(0.01, 0.25),2)
            user_id = message.chat.id
            user = str(user_id)
        ...
        if message.text == "💳 Withdraw":
            ...
            if bal < 1:
                bot.send_message(user_id, "<b>your Balance is too Low to Withdraw🥲🥺 Mnimum Withdraw is 1 rs🙃</b>",parse_mode="html")
      ...
      bot.send_message(
          message.chat.id, "Dear user , you can't withdraw ur requested amount \n\n Possible Reasons: \n\n 1. You have too low balance for Withdraw \n Minimum Withdraw Is 1 Rs \n\n 2. If U have enough balance and not able to withdrae then Contact our Dev : @alien_parii_25")
      return
def broad2(message):
